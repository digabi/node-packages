...
export function fileUploadMiddleware(
  ...
  opts?: { inMemory: true } | { dest: string }
): Handler {
  const storage = opts
    ? 'dest' in opts
      ? multer.diskStorage({ destination: opts.dest })
      : opts.inMemory
        ? multer.memoryStorage()
        : undefined
    : undefined
  const config: multer.Options = { storage, limits: { fileSize: fileMaxSizeBytes } }
...
function fileUpload(config: multer.Options, fieldName: string, errorCode: number, errorMessage: string): Handler {
  ...
  const uploadMiddleware = (req: Request, res: Response, next: NextFunction) => {
    void examUpload(req, res, err => {
      ...
      } else if (err instanceof Error && _.includes(err.message, 'Request aborted')) {
